...
:class:`FigureSpec` -- the lines, their points, and the styling -- and the shared
renderer in :mod:`tools.figures` draws that spec. Nothing draws from the rows
directly, so the spec beside each image is a complete description of it: the
image can be recovered from the data alone, which is what ``cache_mode: reuse``
does.
"""
...
_CONCORDANCE_DIRECTION_COLORS = (CATEGORICAL_COLORS[0], CATEGORICAL_COLORS[1])
_CONCORDANCE_DIRECTION_LINESTYLES = ("-", "--")
_REWARD_PROGRESS_COLOR = CATEGORICAL_COLORS[2]
