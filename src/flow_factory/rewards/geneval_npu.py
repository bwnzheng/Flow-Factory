...
# src/flow_factory/rewards/geneval_npu.py
"""
GenEval Reward Model — device-agnostic variant (CUDA + Ascend NPU).
...
This variant removes the CUDA-only restriction of the original GenEvalRewardModel
by dynamically building device strings from the accelerator.  When MMCV is compiled
with Ascend NPU support (see https://mmdetection.readthedocs.io/en/dev/device/npu.html),
Mask2Former runs transparently on NPU via the same ``init_detector`` / ``inference_detector``
APIs — only the device string changes.

Requirements:
    - mmdet + open_clip_torch (same as geneval.py)
    - For NPU: MMCV compiled for Ascend NPU, ``torch_npu`` installed

Usage (YAML):
    - name: "geneval_reward"
      reward_model: "GenEvalAscend"
      device: "npu"             # or "cuda"
      dtype: float32
"""
...
# ---------------------------------------------------------------------------
# GenEval Ascend Reward Model (device-agnostic: CUDA + NPU)
# ---------------------------------------------------------------------------


class GenEvalAscendRewardModel(PointwiseRewardModel):
    """Compositional T2I reward using Mask2Former detection + CLIP color classification.

    Device-agnostic variant that supports both CUDA and Ascend NPU.
    When MMCV is built for Ascend, Mask2Former runs transparently on NPU devices.
    ...
    def __init__(self, config: RewardArguments, accelerator: Accelerator):
        ...
        # Build device string dynamically from accelerator (supports cuda / npu).
        self._device_type = self.device.type
        self._device_index = self.accelerator.local_process_index
        self._device_str = f"{self._device_type}:{self._device_index}"

        logger.info(
            f"GenEvalAscendRewardModel device: {self._device_str} "
            f"(type={self._device_type}, index={self._device_index})"
        )
        ...
        logger.info(
            f"GenEvalAscendRewardModel initialized: "
    ...
    def _init_detector(self, config: RewardArguments) -> None:
        ...
        logger.info(f"Mask2Former loaded on {self._device_str}.")
    ...
    def _init_clip(self, config: RewardArguments) -> None:
        ...
        self._clip_model, _, self._clip_preprocess = open_clip.create_model_and_transforms(
            clip_model_name, pretrained="openai", device=self._device_str
        )
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        # Device-agnostic autocast: uses self._device_type instead of hard-coded "cuda".
        with torch.amp.autocast(device_type=self._device_type, enabled=False):
