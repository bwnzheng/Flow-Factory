...
`AdvantageProcessor` (`advantage/advantage_processor.py`): communication-aware, auto-selects gather vs local path. Strategies: `"sum"` (GRPO) and `"gdpo"`; shared sample weighting is applied here before trainer-specific optimization. Linear consumers receive the SRC outer multiplier folded into the advantage, while NFT receives a weighted-centered signed-optimality input plus a separate loss multiplier for its complete per-sample regression objective. Logging arrays are globally complete after `_gather_for_logging()` and must be summarized locally rather than rank-reduced again. All reward-based trainers delegate to `self.advantage_processor.compute_advantages()`; the distillation trainer `diffusion-opd` is the exception (its `prepare_feedback()` is a no-op — no reward/advantage stage).
