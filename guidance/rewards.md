...
**`vision_reward`** (key: ``vision_reward``) follows the official image scorer: one prompt-image alignment feature plus the selected yes/no QA features and linear head. Its SAT checkpoint must be merged/extracted into a directory containing ``model_config.json``. The Llama-3 tokenizer is a separate artifact; configure a local directory through ``tokenizer_path`` or ``VISIONREWARD_TOKENIZER``, or select a public compatible Hugging Face repository such as [`hfl/llama-3-chinese-8b-instruct-v3`](https://huggingface.co/hfl/llama-3-chinese-8b-instruct-v3) through ``tokenizer_repo`` (the evaluator downloads only tokenizer/config files). The adapter checks the expected 128,256-token vocabulary before loading weights; Llama-2, Qwen, and Mistral tokenizers are not interchangeable. Public mirrors can have upstream Llama licensing obligations even when their Hugging Face card shows a different license tag, so review the artifact's terms before use. Because the upstream stack pins older PyTorch/Transformers versions, use the [Remote Reward Server](#remote-reward-server) contract when it cannot share the Flow-Factory environment.
