...
| `vision_reward` | Pointwise | VisionReward image preference score via the upstream SAT/CogVLM2 stack; requires an extracted local checkpoint, a local Llama-3 tokenizer, or an isolated reward server | [VisionReward](https://github.com/THUDM/VisionReward) |
...
**`vision_reward`** (key: ``vision_reward``) follows the official image scorer: one prompt-image alignment feature plus the selected yes/no QA features and linear head. Its SAT checkpoint must be merged/extracted into a directory containing ``model_config.json``. The Meta-Llama-3 tokenizer is a separate artifact; configure its local directory through ``tokenizer_path`` or ``VISIONREWARD_TOKENIZER`` for offline evaluation. Because the upstream stack pins older PyTorch/Transformers versions, use the [Remote Reward Server](#remote-reward-server) contract when it cannot share the Flow-Factory environment.
