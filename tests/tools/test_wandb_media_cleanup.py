...
from tools.wandb_media_cleanup import (
    EventCache,
    _pending_files,
    _resolve_entity,
    retry_call,
...
def test_resolve_entity_uses_authenticated_default(monkeypatch):
    class FakeApi:
        default_entity = "authenticated-team"
    ...
    monkeypatch.setattr("tools.wandb_media_cleanup._load_wandb", lambda: FakeWandb())

    assert _resolve_entity(None, 30, 0, 1.0, 10.0) == "authenticated-team"


def test_resolve_entity_preserves_explicit_value(monkeypatch):
    monkeypatch.setattr(
        "tools.wandb_media_cleanup._load_wandb",
        lambda: pytest.fail("W&B should not be loaded for an explicit entity"),
    )

    assert _resolve_entity("explicit-team", 30, 0, 1.0, 10.0) == "explicit-team"
